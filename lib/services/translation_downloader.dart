...
class TranslationDownloader {
  ...
  static final List<Map<String, dynamic>> downloadableTranslations = [
    {'id': 'rus_syn', 'db_id': 'rus_syn', 'lang': 'ru', 'langName': 'Russian', 'name': 'Russian Synodal Bible', 'abbr': 'SYN', 'license': 'Public Domain', 'sizeMB': 5.3},
    {'id': 'cmn_cuv', 'db_id': 'cmn_cuv', 'lang': 'zh', 'langName': 'Chinese', 'name': 'Chinese Union Version', 'abbr': 'CUV', 'license': 'Public Domain', 'sizeMB': 2.4},
    {'id': 'arb_vdv', 'db_id': 'arb_vdv', 'lang': 'ar', 'langName': 'Arabic', 'name': 'Arabic Van Dyck Bible', 'abbr': 'VDV', 'license': 'Public Domain', 'sizeMB': 7.1},
    {'id': 'kor_old', 'db_id': 'kor_old', 'lang': 'ko', 'langName': 'Korean', 'name': 'Korean Bible 1910', 'abbr': 'KOR', 'license': 'Public Domain', 'sizeMB': 4.2},
    {'id': 'ukr_pan', 'db_id': 'ukr_pan', 'lang': 'uk', 'langName': 'Ukrainian', 'name': 'Ukrainian Bible by P. Kulish', 'abbr': 'UKR', 'license': 'Public Domain', 'sizeMB': 5.4},
    {'id': 'HINIRV', 'db_id': 'hinirv', 'lang': 'hi', 'langName': 'Hindi', 'name': 'Hindi Indian Revised Version', 'abbr': 'IRV', 'license': 'CC BY-SA 4.0', 'sizeMB': 8.0},
  ];

  static Future<void> downloadAndInstall(String translationId, void Function(double) onProgress) async {
    final meta = downloadableTranslations.firstWhere((t) => (t['db_id'] ?? t['id']) == translationId);

    // Primary endpoint (actual helloao structure)
    final primaryUrl = "https://bible.helloao.org/api/\${meta['id']}/complete.json";
    ...
    for (int i = 0; i < _maxRetries; i++) {
      try {
        ...
        if (response.statusCode == 200) {
          int totalBytes = response.contentLength ?? (meta['sizeMB'] * 1024 * 1024).toInt();
          int receivedBytes = 0;
    ...
    final parsedData = await Isolate.run(() => _parseTranslationJson(jsonString!, meta));

    // Install to DB
    await bibleDbService.insertTranslationPack(parsedData.info, parsedData.verses);
  }

  // Runs in an isolate
  static _ParsedData _parseTranslationJson(String jsonString, Map<String, dynamic> meta) {
    ...
    if (versesList.isNotEmpty) {
      ...
      for (final v in versesList) {
        ...
        String text = (m['text'] as String).replaceAll('¶ ', '').replaceAll('¶', '').trim().replaceAll('\u2019', "'").replaceAll('\u2018', "'");
        dbVerses.add({
        ...
        for (final chWrap in chapters) {
          ...
          for (final item in content) {
            ...
            if (itemMap['type'] == 'verse') {
              ...
              final text = verseContent.whereType<String>().join('').trim().replaceAll('\u2019', "'").replaceAll('\u2018', "'");
              dbVerses.add({
                'translation_id': info.translationId,
                'language_code': info.languageCode,
                'book_number': bookNum,
