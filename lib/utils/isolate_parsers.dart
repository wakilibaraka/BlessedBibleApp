...
/// Top-level function to parse Uriah Smith / generic commentary JSON on a background isolate
Map<String, Map<String, Map<String, List<CommentaryEntry>>>> parseCommentaryJson(String jsonString) {
  final Map<String, Map<String, Map<String, List<CommentaryEntry>>>> result = {};
  ...
  for (var bookEntry in jsonData.entries) {
    ...
    for (var chapterEntry in chapters.entries) {
      ...
      for (var verseEntry in verses.entries) {
        ...
        final entriesData = verseEntry.value as List<dynamic>;
        for (final e in entriesData) {
          result[book]![chapter]![verse]!.add(CommentaryEntry.fromJson(e as Map<String, dynamic>));
        }
      }
    }
  }
