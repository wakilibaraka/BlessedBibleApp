...
class DayCompleteCelebration extends StatefulWidget {
  ...
  const DayCompleteCelebration({super.key, required this.day, required this.onComplete});

  @override
  State<DayCompleteCelebration> createState() => _DayCompleteCelebrationState();
}

class _DayCompleteCelebrationState extends State<DayCompleteCelebration> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _scaleAnimation = TweenSequence([
      TweenSequenceItem(tween: Tween<double>(begin: 0.5, end: 1.1).chain(CurveTween(curve: Curves.easeOutBack)), weight: 20),
      TweenSequenceItem(tween: Tween<double>(begin: 1.1, end: 1.0).chain(CurveTween(curve: Curves.easeInOut)), weight: 10),
      TweenSequenceItem(tween: Tween<double>(begin: 1.0, end: 1.0), weight: 50),
      TweenSequenceItem(tween: Tween<double>(begin: 1.0, end: 0.8).chain(CurveTween(curve: Curves.easeIn)), weight: 20),
    ]).animate(_controller);
                ...
                child: CustomPaint(
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.stars_rounded, color: Theme.of(context).primaryColor, size: 64),
...
class _SparklePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final paint = Paint()
      ..color = const Color(0xFFFFD700).withValues(alpha: 1.0 - progress) // Gold
