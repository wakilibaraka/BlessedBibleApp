...
class CreateCustomPlanScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CreateCustomPlanScreen> createState() =>
      _CreateCustomPlanScreenState();
}

class _CreateCustomPlanScreenState
    extends ConsumerState<CreateCustomPlanScreen> {
  ...
  void _createPlan(CustomPlanScheduler scheduler) {
    ...
    if (title.isEmpty) {
      if (_corpusType == 'whole') {
        title = 'My Bible Plan';
      } else if (_corpusType == 'ot') {
        title = 'My OT Plan';
      } else if (_corpusType == 'nt') {
        title = 'My NT Plan';
      } else if (_corpusType == 'book') {
        title = '$_selectedBook Plan';
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    final bookCorpus = scheduler.buildCorpus('book',
        startBook: _selectedBook, startChapter: 1);
    final sliceCorpus = scheduler.buildCorpus('slice',
        startBook: _selectedBook, startChapter: _selectedChapter);
    final multiCorpus =
        scheduler.buildCorpus('multi', selectedBooks: _selectedBooks);
    ...
    double avgPerDay =
        totalReadingDays > 0 ? (corpus.length / totalReadingDays) : 0;
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildIOSBox([
            _buildCheckRow('Whole Bible', _corpusType == 'whole',
                () => setState(() => _corpusType = 'whole'), theme,
                subtitle: 'Genesis to Revelation · 1189 chapters'),
            _buildCheckRow('Old Testament', _corpusType == 'ot',
                () => setState(() => _corpusType = 'ot'), theme,
                subtitle: 'Genesis to Malachi · 929 chapters'),
            _buildCheckRow('New Testament', _corpusType == 'nt',
                () => setState(() => _corpusType = 'nt'), theme,
                subtitle: 'Matthew to Revelation · 260 chapters'),
            _buildCheckRow(
                _selectedBook != null && _corpusType == 'book'
                    ? '$_selectedBook'
                    : 'Choose a book',
                _corpusType == 'book', () {
              setState(() => _corpusType = 'book');
            }, theme,
                subtitle: _selectedBook != null && _corpusType == 'book'
                    ? '${bookCorpus.length} chapters'
                    : 'e.g. Genesis · 50 chapters'),
            if (_corpusType == 'book')
              _buildIOSRow('Select Book', value: _selectedBook ?? 'None',
                  onTap: () {
                _showBookPicker(
                    context,
                    bibleState.books.map((b) => b.name).toList(),
                    (val) => setState(() => _selectedBook = val));
              }, theme: theme),
            _buildCheckRow(
                _selectedBook != null &&
                        _selectedChapter != null &&
                        _corpusType == 'slice'
                    ? 'From $_selectedBook $_selectedChapter to Revelation 22'
                    : 'Start from a chapter...',
                _corpusType == 'slice', () {
              setState(() => _corpusType = 'slice');
                ...
                subtitle: _selectedBook != null &&
                        _selectedChapter != null &&
                        _corpusType == 'slice'
                    ? '${sliceCorpus.length} chapters'
                    : 'e.g. from Isaiah 47 to the end'),
            if (_corpusType == 'slice') ...[
              _buildIOSRow('Start Book', value: _selectedBook ?? 'None',
                  onTap: () {
                _showBookPicker(
                    context,
                    bibleState.books.map((b) => b.name).toList(),
              ...
              if (_selectedBook != null)
                _buildIOSRow('Start Chapter',
                    value: 'Chapter ${_selectedChapter ?? 1}', onTap: () {
                  int count = bibleState.books
                      .firstWhere((b) => b.name == _selectedBook,
                          orElse: () => bibleState.books.first)
                      .chapters
                      .length;
                  _showChapterPicker(context, count,
                      (val) => setState(() => _selectedChapter = val));
                }, theme: theme),
            ],
            _buildCheckRow(
                _selectedBooks.isNotEmpty && _corpusType == 'multi'
                    ? _getMultiBookSummary()
                    : 'Choose books',
                _corpusType == 'multi', () {
              setState(() => _corpusType = 'multi');
            }, theme,
                subtitle: _selectedBooks.isNotEmpty && _corpusType == 'multi'
                    ? '${multiCorpus.length} chapters'
                    : 'e.g. Genesis, Exodus · 90 chapters'),
            if (_corpusType == 'multi')
              _buildIOSRow('Select Books',
                  value: '${_selectedBooks.length} selected', onTap: () {
                _showMultiBookPicker(
                    context, bibleState.books.map((b) => b.name).toList());
              }, theme: theme),
          ], theme, surfaceColor: getThemeSurfaceColor()),
          _buildSectionHeader('How long', theme),
          _buildIOSBox([
            _buildCheckRow('By Duration', _useDuration,
                () => setState(() => _useDuration = true), theme),
            if (_useDuration)
              _buildIOSInputRow(
                  ...
                  theme: theme),
            _buildCheckRow('By Target Date', !_useDuration,
                () => setState(() => _useDuration = false), theme),
            if (!_useDuration)
              _buildIOSRow('End Date',
                  value:
                      "${_targetEndDate.year}-${_targetEndDate.month.toString().padLeft(2, '0')}-${_targetEndDate.day.toString().padLeft(2, '0')}",
                  onTap: () async {
          ...
          _buildIOSBox([
            _buildCheckRow('Saturday', _restDay == 7,
                () => setState(() => _restDay = 7), theme),
            _buildCheckRow(
                _restDay != 7 && _restDay != null
                    ? _restDayName(_restDay)
                    : 'Choose a day...',
                _restDay != 7 && _restDay != null, () {
              _showRestDayPicker();
            }, theme),
            _buildCheckRow('None', _restDay == null,
                () => setState(() => _restDay = null), theme),
          ], theme, surfaceColor: getThemeSurfaceColor()),
          ...
          _buildIOSBox([
            _buildCheckRow(
                ...
                _paceMode == 'scheduled',
                () => setState(() => _paceMode = 'scheduled'),
                theme),
            _buildCheckRow(
                'Flexible (Read at own pace)',
                _paceMode == 'flexible',
                () => setState(() => _paceMode = 'flexible'),
                theme),
          ], theme, surfaceColor: getThemeSurfaceColor()),
          const SizedBox(height: 24),
          _buildPreviewBox(corpus, isValid, avgPerDay, totalReadingDays,
              requestedReadingDays, theme),
        ],
      ),
    ...
    return Padding(
      ...
      child: Container(
        decoration: BoxDecoration(
          color:
              surfaceColor ?? theme.colorScheme.surface.withValues(alpha: 0.6),
  ...
  Widget _buildCheckRow(
      String title, bool selected, VoidCallback onTap, ThemeData theme,
      {String? subtitle}) {
    return InkWell(
      onTap: onTap,
      borderRadius: BorderRadius.circular(16),
  ...
  Widget _buildIOSRow(String title,
      {String? value, VoidCallback? onTap, ThemeData? theme}) {
    return InkWell(
      onTap: onTap,
      borderRadius: BorderRadius.circular(16),
      child: Padding(
  ...
  Widget _buildPreviewBox(List<dynamic> corpus, bool isValid, double avgPerDay,
      int totalReadingDays, int requestedReadingDays, ThemeData theme) {
    String planNameStr =
        _corpusType == 'book' ? (_selectedBook ?? 'This plan') : 'This plan';
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16.0),
          ...
          child: Column(
            ...
            children: [
              ...
              if (corpus.isEmpty)
                ...
                const Text('Please enter a valid timeframe (1 to 1095 days).',
                ...
                const SizedBox(height: 4),
                Text(
                    '• ~${avgPerDay.toStringAsFixed(1)} chapters per reading day',
                    style: theme.textTheme.bodyMedium),
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                              child: Text(
                                  '$planNameStr has ${corpus.length} chapters — this plan will run $totalReadingDays reading days (shorter than the $requestedReadingDays you entered).',
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                              child: Text(
                                  'Intensive — about ${avgPerDay.toStringAsFixed(0)} chapters/day.',
                                  style: const TextStyle(
                                      color: Colors.orange,
                                      fontWeight: FontWeight.bold,
                                      fontSize: 13))),
                ...
                if (avgPerDay < 0.2)
                  Padding(
                    padding: const EdgeInsets.only(top: 12.0),
                    child: Text(
                        'Note: This plan is very relaxed, with many rest days.',
                        style: theme.textTheme.labelSmall?.copyWith(
  ...
  void _showBookPicker(BuildContext context, List<String> books,
  ...
  void _showChapterPicker(
      BuildContext context, int chapterCount, ValueChanged<int> onSelected) {
    showCupertinoModalPopup(
      context: context,
      builder: (_) => Container(
        height: 250,
        ...
        child: CupertinoPicker(
          ...
          scrollController: FixedExtentScrollController(
              initialItem: (_selectedChapter ?? 1) - 1),
