...
def fetch_node(node_id):
    url = f"https://m.egwwritings.org/en/book/1391.{node_id}"
    ...
    # EGW navigation uses <li class="next"><a href="/en/book/1391.24"...>
    next_match = re.search(r'<li class=\"next\"><a[^>]*href=\"/en/book/1391\.(\d+)\"', html)
    if not next_match:
        # Fallback: look for an 'a' tag with text 'Next'
        next_match = re.search(r'href=\"/en/book/1391\.(\d+)\"[^>]*>\s*Next\s*</a>', html, re.IGNORECASE)
        
    next_node = next_match.group(1) if next_match else None
    
    return texts, next_node

start_nodes = [17, 222, 396, 677, 831, 1001]
end_nodes = [222, 396, 677, 831, 1001, 999999]
...
for ch in range(1, 7):
    ...
    with open(f"scripts/glad_tidings_ch{ch}.txt", "w") as f:
        f.write("\n\n".join(ch_text))
